...
html, body, [class*="css"] { color: #2F2A25; }
.disclaimer-card {
    padding: 1rem;
    border-radius: 18px;
...
.step-active, .step-complete, .step-inactive {
    width: 130px;
    min-height: 42px;
...
.step-active { background: #7A9E7E; color: white; }
.step-complete { background: #DCEBD8; color: #355E3B; }
.step-inactive { background: #EFE8DD; color: #7A6A58; }
.result-card {
    padding: 1.4rem;
    border-radius: 22px;
...
BREEDS = [
    ...
    "toy / companion breed",
    "brachycephalic / short-nosed breed",
    "hound / scent breed",
    "sporting / retriever breed",
    "terrier breed",
    "guardian / large breed",
]
...
session_defaults = {
    ...
    "dog_name": "Sadie",
    "age_group": "adult",
    "breed_type": "herding / working breed",
    "size": "medium",
    "energy": "high",
...
def evidence_scores(inputs: dict):
    ...
    breed = inputs.get("breed_type", "mixed / unknown")
    ...
    # Breed personalization
    if breed == "herding / working breed":
        scores["boredom"] += 0.5
        scores["overstimulation"] += 0.4
        if behavior in ["pacing", "toy-seeking"] and activity in ["none", "low"]:
            scores["boredom"] += 0.9
    elif breed == "toy / companion breed":
        scores["anxiety"] += 0.3
        if environment == "noisy":
            scores["anxiety"] += 0.4
    elif breed == "brachycephalic / short-nosed breed":
        if behavior == "panting" or mouth == "open mouth / panting":
            scores["needs observation"] += 0.8
            scores["recovery"] += 0.4
    elif breed == "hound / scent breed":
        if behavior == "pacing" and activity in ["none", "low"]:
            scores["boredom"] += 0.5
    elif breed == "sporting / retriever breed":
        if activity == "high":
            scores["recovery"] += 0.4
            scores["overstimulation"] += 0.3
    elif breed == "terrier breed":
        if behavior in ["pacing", "toy-seeking"]:
            scores["boredom"] += 0.3
            scores["overstimulation"] += 0.3
    elif breed == "guardian / large breed":
        if environment == "noisy" or behavior == "pacing":
            scores["anxiety"] += 0.2
    ...
    scores = {key: max(0.01, value) for key, value in scores.items()}
    total = sum(scores.values())
    probs = {key: value / total for key, value in scores.items()}
    return probs, scores
...
def hybrid_predict(inputs, ml_model):
    ...
    for cls, prob in zip(ml_classes, ml_raw_probs):
        ml_probs[cls] = float(prob)

    evidence_probs, raw_scores = evidence_scores(inputs)
    final_probs = {c: 0.78 * evidence_probs[c] + 0.22 * ml_probs.get(c, 0) for c in CLASSES}
...
def generate_data(n=3000):
    ...
    rows = []
    for _ in range(n):
        row = {key: np.random.choice(value) for key, value in values.items()}
...
def analyze_image_with_ai(uploaded_image):
    ...
    prompt = """
Return valid JSON only. Determine whether the image clearly shows a dog.
If it does not clearly show a dog, return image_valid=false.
...
def factors(inputs):
    ...
    if inputs["movement_pattern"] == "repetitive pacing":
        risk.append("Repeated pacing pattern")
    if inputs["movement_level"] == "high movement":
        risk.append("High movement level")
    if inputs["behavior_change"] == "worsening / escalating":
        risk.append("Behavior worsening over time")
    if inputs["behavior_continuity"] == "continuous":
        risk.append("Continuous behavior")

    if inputs["activity"] == "high" and (inputs["behavior"] == "panting" or inputs["visual_mouth"] == "open mouth / panting"):
    ...
    if inputs["movement_pattern"] == "settling down":
        protective.append("Dog appears to be settling down")
    if inputs["behavior_change"] == "improving / settling":
...
def build_reasoning(inputs, visual_quality_score, temporal_quality_score):
    dog = st.session_state.get("dog_name", "your dog")
    reasons = []

    if inputs["media_type"] == "video":
        reasons.append(f"{dog}'s video-assisted cues were included, so the interpretation considers both visible body-language signals and behavior over time.")
    elif inputs["media_type"] == "image":
        reasons.append(f"{dog}'s image-assisted cues were included, so the interpretation considers visible body-language signals.")
    else:
        reasons.append(f"No media was used for {dog}. The interpretation is based on profile and behavior context only.")

    if inputs.get("breed_type") == "herding / working breed":
        reasons.append(f"{dog}'s breed type can make pacing or toy-seeking more relevant when stimulation is low.")
    if inputs.get("breed_type") == "brachycephalic / short-nosed breed":
        reasons.append(f"{dog}'s breed type makes panting more important to monitor carefully.")
    ...
    reasons.append(f"Visual interpretation quality is {quality_label(visual_quality_score)}.")
    reasons.append(f"Temporal interpretation quality is {quality_label(temporal_quality_score)}.")
    return reasons
...
def what_would_change_prediction(inputs):
    ...
    if inputs["behavior_change"] == "unknown":
...
with st.sidebar:
    ...
    with st.expander("How AI is used", expanded=True):
        st.write(
            ...
            2. A behavioral evidence scoring layer calibrates cues like whale eye, tucked tail, relaxed posture, breed type, activity, and movement over time.
            3. Optional image AI can validate dog images and pre-fill visual cues when API credits are available.
            4. Video-assisted review samples frames locally and lets the user review temporal cues.
    ...
    with st.expander("Data transparency"):
        st.write(
            ...
            Feedback is logged for future retraining but does not automatically retrain after each submission.
            """
        )
...
st.write(
    "A preventive AI decision-support prototype that helps owners understand ambiguous dog behavior using profile, breed type, context, and optional image/video cues."
)
...
if st.session_state.phase == 1:
    ...
    with c1:
        st.session_state.dog_name = st.text_input(
            "Dog name",
            value=st.session_state.dog_name,
            help="The name is used throughout the interpretation to personalize the guidance.",
        )
        st.session_state.age_group = st.selectbox(
            "Age group",
            ["puppy", "adult", "senior"],
            index=["puppy", "adult", "senior"].index(st.session_state.age_group),
        )

    with c2:
        st.session_state.breed_type = st.selectbox(
            "Breed type",
            BREEDS,
            index=BREEDS.index(st.session_state.breed_type),
            help="Breed type helps contextualize likely behavior patterns. For example, herding breeds may show restlessness when under-stimulated.",
        )
        st.session_state.size = st.selectbox(
            "Size category",
            ["small", "medium", "large"],
            index=["small", "medium", "large"].index(st.session_state.size),
        )
    ...
    with c3:
        st.session_state.energy = st.selectbox(
            ...
            ["low", "moderate", "high"],
            index=["low", "moderate", "high"].index(st.session_state.energy),
        )
        st.session_state.sensitivity = st.selectbox(
            "Known stress sensitivity",
            ["low", "moderate", "high"],
            index=["low", "moderate", "high"].index(st.session_state.sensitivity),
        )

    st.session_state.assumption = st.selectbox(
        f"What do you think is happening with {st.session_state.dog_name}?",
        ["unsure", "anxiety", "boredom", "overstimulation", "recovery"],
        index=["unsure", "anxiety", "boredom", "overstimulation", "recovery"].index(st.session_state.assumption),
    )

    nav1, _, spacer = st.columns([1, 1, 3])
    with nav1:
        st.button(f"Next: {st.session_state.dog_name}'s Context", type="primary", on_click=set_phase, args=(2,), use_container_width=True)
...
# ==================================================
    ...
    dog = st.session_state.dog_name
    st.markdown(f"## Phase 2 — What is {dog} doing?")

    c1, c2 = st.columns(2)

    with c1:
        st.session_state.behavior = st.selectbox(
            f"What behavior do you observe in {dog}?",
            ["panting", "pacing", "whining", "hiding", "toy-seeking", "resting"],
            index=["panting", "pacing", "whining", "hiding", "toy-seeking", "resting"].index(st.session_state.behavior),
        )
        st.session_state.activity = st.selectbox(
            f"What was {dog}'s recent activity?",
            ["none", "low", "high"],
            index=["none", "low", "high"].index(st.session_state.activity),
        )

    with c2:
        st.session_state.environment = st.selectbox(
            f"What environment is {dog} in?",
            ["indoor", "outdoor", "warm", "noisy"],
            index=["indoor", "outdoor", "warm", "noisy"].index(st.session_state.environment),
        )
        st.session_state.duration = st.selectbox(
            f"How long has {dog} been doing this?",
            ["short", "medium", "long"],
            index=["short", "medium", "long"].index(st.session_state.duration),
        )
    ...
    with nav2:
        st.button(f"Next: {dog}'s Visual Cues", type="primary", on_click=set_phase, args=(3,), use_container_width=True)
...
# ==================================================
    ...
    dog = st.session_state.dog_name
    st.markdown(f"## Phase 3 — {dog}'s Image / Video Cue Review")

    st.write(
        f"Upload a clear image or short video of {dog} if available. Video adds temporal signals like repeated pacing, settling, or escalation over time."
    )

    media_choice = st.radio("Input type", ["No media", "Image", "Video"], horizontal=True)
    invalid_media = False

    if media_choice == "Image":
        image = st.file_uploader(f"Upload an image of {dog}", type=["jpg", "jpeg", "png"])

        if image:
            st.image(image, caption=f"Uploaded image of {dog}", width=350)
            ...
            if st.button("Analyze Image Cues with AI", type="secondary"):
                with st.spinner(f"Analyzing {dog}'s image cues..."):
                ...
                if "error" in image_result:
                    ...
                    st.error(f"Invalid image. Please upload an image that clearly shows {dog} or another dog.")
                    st.session_state.image_available = "no"
                    st.session_state.media_type = "none"
                    invalid_media = True

                else:
                    st.success(f"{dog}'s image was validated. Visual cues were auto-filled.")
        ...
        video = st.file_uploader(f"Upload a short video of {dog}", type=["mp4", "mov", "avi", "m4v"])
    ...
    st.markdown(f"### {dog}'s visual cues")
    c1, c2, c3 = st.columns(3)
    ...
    st.markdown(f"### {dog}'s movement over time")
    t1, t2 = st.columns(2)
    ...
    visual_quality = visual_cue_completeness(st.session_state.image_available, st.session_state.video_available, *visuals)
    ...
    with nav2:
        analyze_clicked = st.button(f"Analyze {dog}", type="primary", use_container_width=True, disabled=invalid_media)
    ...
    if analyze_clicked:
        inputs = {
            ...
            "breed_type": st.session_state.breed_type,
...
# ==================================================
    ...
    dog = st.session_state.dog_name
    st.markdown(f"## Phase 4 — {dog}'s AI Interpretation")
    ...
    if result is None:
        ...
        for key in ["visual_mouth", "visual_posture", "visual_ears", "visual_tail", "visual_eyes", "visual_hiding", "movement_pattern", "movement_level", "behavior_change", "behavior_continuity"]:
            context_only_inputs[key] = "unknown"
        ...
        with r1:
            ...
            st.write(f"**Dog:** {dog}")
            st.write(f"**Breed type:** {inputs.get('breed_type', 'mixed / unknown')}")
        ...
        with r2:
            ...
            st.write(rec["summary"].replace("your dog", dog))
            for step in rec["do_now"][:3]:
                st.write(f"- {step.replace('your dog', dog)}")
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        st.info(
            f"This does not mean {dog} is definitely {pred}. "
        ...
        with tabs[0]:
            ...
            st.markdown(f"### Clear action plan for {dog}")
            ...
            with a4:
                ...
                for step in ["Labored breathing", "Collapse or pale gums", "Vomiting or severe distress", "Symptoms worsen or persist"]:
                    st.write(f"- {step}")
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        with tabs[2]:
            ...
            if inputs["media_type"] == "video":
                st.success(f"{dog}'s video cues were included, adding movement pattern, movement level, change over time, and behavior continuity.")
            elif inputs["media_type"] == "image":
                st.success(f"{dog}'s image cues were included, adding visible body-language signals.")
            else:
...
# ==================================================
    ...
    dog = st.session_state.dog_name
    st.markdown(f"## Phase 5 — Feedback for {dog}")
    st.write(f"Feedback helps demonstrate how the prototype could learn from {dog}'s real-world outcomes. You can also start a new interpretation without submitting feedback.")
    ...
    if result is None:
        ...
        with st.form("feedback_form"):
            ...
            with fb1:
                accurate = st.radio(f"Was this accurate for {dog}?", ["Yes", "No", "Unsure"], horizontal=True)
            with fb2:
                action_taken = st.radio("Did you take the action?", ["Yes", "No", "Partially"], horizontal=True)
            with fb3:
                improved = st.radio(f"Did {dog}'s behavior improve?", ["Yes", "No", "Not sure"], horizontal=True)
